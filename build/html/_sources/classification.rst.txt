...
location, item type, collection code, and call number -- in that order.

A visual overview of how items are classified would look something like
this:

.. image:: images/010-classification-opac.jpg

Home library:
-------------

Each item ***must*** include a home library that is defined by the
...
Each item record *should* include a shelving location that is defined by
...
Each item record ***must*** include an item type that is defined by the
...
Each item record *should* include a collection code that is defined by
...
Each item record *should* include a call number. Call numbers are not
...
080\ :math:`a field, the system will attempt to use that number as an item's default call number. If, however, nothing is defined in the 080`\ a,
